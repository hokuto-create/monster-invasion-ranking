...
通知が送られるたびに、GitHub Actions の各 run ページ上部の **Summary** に送信結果が表示されます。
...
- **送信成功（受信した人数）** = `sent`: プッシュサービスが受理した件数（実際に届いた人数の目安）。
- **購読総数** = `total`: KV に保存されている購読の総数。
- **無効として削除** = `removed`: 期限切れ等で無効だった購読を自動削除した件数。

`Actions` タブ → 該当の `Push notification on update` の run を開くと確認できます。
